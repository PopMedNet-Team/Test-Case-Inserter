...
extends Database {
    ...
    private static String fkTempl = "alter table FKTABLE add constraint fk_FKTABLE_PKTABLE foreign key (FKCOL) references PKTABLE (PKCOL) on update no action on delete no action;";
    private static String uniqueTempl = "alter table TABLE add constraint uc_TABLE_COL unique (COL);";
    private static String notNullTempl = "alter table TABLE alter column COL set not null;";
    ...
    public String dropOneTable(DbmsTable tab) {
        ...
        sb.append("drop table if exists " + tab.getTableName());
        return sb.toString();
    }
    ...
    public String createSequence() {
        return "create sequence tci_seq start 1";
    }

    @Override
    public String dropSequence() {
        return "drop sequence if exists tci_seq";
    }

    @Override
    public String getSequenceQuery() {
        String sql = "select nextval('" + this.connProps[3] + "." + "tci_seq" + "') ";
        return sql;
    }
